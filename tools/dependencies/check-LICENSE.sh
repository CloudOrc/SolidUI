...
#
mkdir dist || if exist true

tar -zxf target/solidui*-bin.tar.gz --strip=1 -C dist

# List all modules(jars) that belong to the SolidUI itself, these will be ignored when checking the dependency
# licenses
echo '=== Self modules: ' && ./mvnw.cmd --batch-mode --quiet -Dexec.executable='echo' -Dexec.args='${project.artifactId}-${project.version}.jar' exec:exec | tee self-modules.txt
...
# [solidui/release-docs/LICENSE] and [solidui/release-docs/licenses/]
